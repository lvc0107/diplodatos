...
## Exploracion de variables individuales 5:
  * 6 Densidad
```{r echo=TRUE}
plot(density(impute(iris$Sepal.Length, mean)))
```
...
```{r}
arr_2hs <- subset(flights, arr_delay > 120)
arr_2hs[,c(3,2,1,11)]
```

- Volaron hacia Houston (IAH o HOU)
```{r}
dest_hou <- subset(flights, dest == 'HOU' | dest == 'IAH')
head(dest_hou)
```

- Fueron operados por United, American o Delta.
```{r}
car_uad <- subset(flights,carrier == 'DL' | carrier == 'AA' | carrier == 'UA' )
head(car_uad)
```

- Salieron en Verano (Julio, Agosto y Septiembre)
```{r}
month_jas <- subset(flights, month == 7 | month == 8 | month == 9 )
head(month_jas)
...
```{r}
arr_sched_delay <- subset(flights, arr_delay > 120  & dep_delay == 0 )
head(arr_sched_delay)

```

- Salieron entre medianoche y las 6 am.
```{r}
dep_mid_six <- subset(flights, dep_time < 600 & dep_time > 0  )
head(dep_mid_six)
...
arribaron con un retraso de mas de dos horas.
```{r}
arr_delay_more2 <- subset(flights, arr_delay > 120)
head(arr_delay_more2)
